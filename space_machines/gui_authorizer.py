...
test_keys = {
  7899246 : {"status":"ok","message":{"user_login":"JSmith","ID":"18706","display_name":"Johnny","account_balance":"12.78"}, "valid":True},# format that the url responds with
  7902312 : {"status":"ok","message":{"user_login":"JDoe","ID":"63150","display_name":"Jane","account_balance":"10.00"}, "valid":False},
  ...
  4702055 : {"status":"ok","message":{"user_login":"BKarstens","ID":"81544","display_name":"Barrett","account_balance":"0.52"}, "valid":False},
  1234567890 : {"status":"ok","message":{"user_login":"CMeyer","ID":"77557","display_name":"Chris","account_balance":"-100.00"}, "valid":True},
...
class GuiAuthorizer(StateMachine):
  ...
  def isRFIDAuthorized(self, key):
    try:
      ...
      self.log.info('Error when checking validity {0}'.format(str(e)))
      try:
        if self.isvalid.get():
          return True
        else:
